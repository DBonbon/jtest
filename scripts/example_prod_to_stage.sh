...
readonly STAGE_HOST=deploy@stage.openline.site
readonly PROD_HOST=deploy@openline.site
readonly REMOTE_MEDIA_PATH=/mnt/persist/www/quartets/shared/media
...
ssh $PROD_HOST "pg_dump -h localhost -Fc -f /tmp/db-dump.sql -U postgres postgres -x -O"
...
ssh $STAGE_HOST "sudo -u postgres pg_restore --clean -h localhost -d postgres -U postgres '/tmp/db-dump.sql'"
ssh $STAGE_HOST "rm /tmp/db-dump.sql"

rm /tmp/db-dump.sql

echo "Updating database..."
manage_prefix="source /mnt/persist/www/quartets/shared/venv/bin/activate && cd /mnt/persist/www/quartets/current/src &&"

ssh $STAGE_HOST "$manage_prefix python manage.py change_site_domain --site_id=1 --new_site_domain='stage.openline.site'"
