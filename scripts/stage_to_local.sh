...
readonly LOCAL_DOMAIN=openline.site.test:8081
readonly SSH_HOST=deploy@stage.openline.site
...
ssh $SSH_HOST "export PGUSER=postgres && pg_dump postgres --no-owner > /tmp/db-dump.sql"
